    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <p className="text-sm text-zinc-400">
          Deleting this vault will permanently remove all categories and bookmarks associated with
          it. This action cannot be undone.
        </p>
        {state?.message && (
          <p className="text-sm text-red-400 bg-red-500/10 p-3 rounded-lg">{state.message}</p>
        )}
      </div>
            ...
            <AlertDialogHeader>
              ...
              <div className="py-4 space-y-2">
                <p className="text-sm text-muted-foreground">
                  Please type{" "}
                  <span className="font-mono font-bold select-all">delete {vaultName}</span> to
                  confirm.
                </p>
                <Input
                  value={confirmText}
