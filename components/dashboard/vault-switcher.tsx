...
import { Box, Briefcase, Check, ChevronsUpDown, Lock, Plus } from "lucide-react";
    ...
    <div className="relative mb-8">
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-8 h-8 rounded-lg flex items-center justify-center bg-zinc-900 border border-zinc-800 group-hover:border-zinc-700 transition-colors`}
          >
            {activeVault ? (
          ...
          {vaults.map((vault, index) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-2 py-2 rounded-lg text-sm transition-all mb-0.5 ${
                  isActive
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${style.color} ${isActive ? style.shadow : ""}`}
                  />
