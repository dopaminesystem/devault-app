...
export function CategorySettingsDialog({ category, open, onOpenChange }: CategorySettingsDialogProps) {
    const [updateState, updateAction, isUpdatePending] = useActionState(updateCategory, initialState);
    const [deleteState, deleteAction, isDeletePending] = useActionState(deleteCategory, initialState);
    const [name, setName] = useState("");
    const [confirmDelete, setConfirmDelete] = useState(false);

    // Initialize form with category data when opened
    useEffect(() => {
        if (category && open) {
