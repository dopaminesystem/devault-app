...
import { updateCategory, deleteCategory } from "@/app/actions/category";
...
export function CategorySettingsDialog({ category, open, onOpenChange }: CategorySettingsDialogProps) {
    return (
...
function CategorySettingsContent({ category, onOpenChange }: { category: Category, onOpenChange: (open: boolean) => void }) {
    const [updateState, updateAction, isUpdatePending] = useActionState<ActionState, FormData>(updateCategory, initialState);
    const [deleteState, deleteAction, isDeletePending] = useActionState<ActionState, FormData>(deleteCategory, initialState);
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <Input
                            ...
                            autoFocus
                        />
                        <Button type="submit" disabled={isUpdatePending || !name.trim() || name === category.name} size="sm">
            ...
            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 space-y-3">
                ...
                {!confirmDelete ? (
                    ...
                    <form action={deleteAction} className="space-y-2">
                        <input type="hidden" name="categoryId" value={category.id} />
                        <p className="text-xs text-red-300">Are you sure? This effectively deletes all bookmarks in this category.</p>
                        <div className="flex gap-2">
                            <Button
                                type="button"
                                variant="outline"
