...
import { subscribeToVault } from "@/modules/vault/actions/vault-member.actions";
import { ShareModal } from "@/modules/vault/components/share-modal";
...
export function VaultHeader({
  ...
  const callbackUrl = useMemo(() => encodeURIComponent(`/v/${vaultSlug}`), [vaultSlug]);
      ...
      <div className="flex flex-col items-start justify-center mb-10 space-y-6">
        ...
        <div className="w-full flex flex-col md:flex-row gap-6 md:items-end justify-between">
          ...
          <div>
            ...
            <p className="text-zinc-500 mt-2 text-sm">Showing {bookmarkCount} bookmarks</p>
          </div>

          {/* Actions Row */}
          <div className="flex items-center gap-3 w-full max-w-md">
            {showSubscribeButton && (
              <SubscribeButton isLoadingSubscription={isSubscribing} onClick={handleSubscribe} />
            )}
            {/* Unsubscribe for non-owner members */}
            {!isOwner && isMember && <UnsubscribeButton vaultId={vaultId} vaultName={vaultName} />}
