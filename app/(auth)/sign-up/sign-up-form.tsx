...
export default function SignUpForm() {
  ...
  const [, enableAction, enablePending] = useActionState(enableEmailSignInAction, {
    error: "",
    success: false,
  });
      ...
      <div className={`${TOKENS.panel} w-full max-w-sm p-6 sm:p-8 space-y-6 z-10`}>
        <div className="flex items-center gap-4 border-b border-zinc-800/50 pb-2 mb-2">
          <Link
            ...
            className={`pb-2 text-sm font-medium transition-colors relative ${
              !isRegister ? "text-zinc-100" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
            Log In
          </Link>
        ...
        <form action={formAction} className="space-y-4">
          <div className="space-y-1.5">
            <label htmlFor="signup-name" className="text-xs font-medium text-zinc-400 ml-1">
              Full Name
            </label>
            <input
              id="signup-name"
          ...
          <div className="space-y-1.5">
            <label htmlFor="signup-email" className="text-xs font-medium text-zinc-400 ml-1">
              Email
            </label>
            <input
              id="signup-email"
              ...
              placeholder="design@engineer.com"
              className={TOKENS.input}
              required
            />
          ...
          <div className="space-y-1.5">
            <label htmlFor="signup-password" className="text-xs font-medium text-zinc-400 ml-1">
              Password
            </label>
            <input
              id="signup-password"
        ...
        <AlertDialogContent className="bg-zinc-950 border-zinc-800 text-zinc-100">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-zinc-400">
              {state?.message ||
                "There is already an account associated with this email via Discord."}
              <br />
              <br />
              Do you want to enable password sign-in for this account? This will set the password
              you just entered.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
